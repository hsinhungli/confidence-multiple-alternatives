function [nllk] = nll_map_is(x,trl,stiPar,config_pool,condIdx)
%--Max model with both inference noise and sensory noise--
% This scripts use Monte-Carlo simualtions to estimate response distribution
...
for config = config_pool
    ...
    [sim_c_map,sim_d] = max(sim_post,[],3); %MAX confidence and category decision

    %Get probability map given a set of parameter value
    [~,sim_r]  = histc(sim_c_map,k,1); %transform internal confidnece to 4-point confidence report based on k
